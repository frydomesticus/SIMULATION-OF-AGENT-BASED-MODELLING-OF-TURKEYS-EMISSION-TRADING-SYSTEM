...
def veri_yukle():
    ...
    Yüklenen Tablolar:
    ------------------
    1. ulusal_envanter: Yıllık sektörel emisyon verileri (NIR 2024)
    2. il_katsayilari: 81 il dağılım katsayıları (TÜİK 2023)
    
    Returns:
    --------
    tuple: (df_envanter, df_il)
    
    Kaynak: database_setup_v2.py tarafından oluşturulur.
    """
...
@st.cache_data(ttl=3600)
def tesis_verisi_yukle():
    """
    Veritabanından tesis listesini yükler.
    
    Bu fonksiyon, TEİAŞ ve EPDK verilerine dayanan ETS kapsamındaki
    büyük tesislerin listesini döndürür.
    
    Returns:
    --------
    pd.DataFrame: Tesis listesi (40 santral)
    
    Sütunlar:
    ---------
    - Tesis_ID, Tesis_Adi, Il, Yakit_Tipi
    - Kapasite_MW, Yillik_Emisyon_tCO2, ETS_Kapsami
    
    Kaynak: TEİAŞ (2024), EPDK (2024), data/tesisler.csv
    """
    if not os.path.exists(DB_PATH):
        return pd.DataFrame()
    ...
    conn = sqlite3.connect(DB_PATH)
    try:
        # Tesisler tablosu var mı kontrol et
        cursor = conn.cursor()
...
def ai_baseline_yukle():
    ...
    Bu fonksiyon, ai_tahmin_v2.py tarafından oluşturulan
    ai_baseline.json dosyasını okur.
    
    Returns:
    --------
    dict: AI baseline verileri veya None
    
    İçerik:
    -------
    - years: [2026, 2027, ..., 2035]
    - emissions: BAU projeksiyon tahminleri
    - model_r2: AI model R² skoru
    - simulation_params: Simülasyon başlangıç değerleri
    
    Kaynak: ai_tahmin_v2.py → output/ai_baseline.json
    """
    import json
...
def sankey_diagram_olustur(df_envanter, son_yil):
    ...
    # Kaynak payları [TEİAŞ 2024 & NIR 2024 Güncel Veriler]
    FOSIL_PAYI = 0.62       # Enerji sektöründe fosil yakıt payı (Elektrik üretimi ~%62)
    ENDUSTRI_PAYI = 0.28    # Endüstriyel enerji kullanımı payı (~%28)
    YENILENEBILIR_PAYI = 0.10  # Yenilenebilir/Diğer payı (Isıl + Elektrik ~%10)
...
# =============================================================================
# GLOBAL DEĞERLER (Sidebar kaldırıldı, varsayılan değerler)
# =============================================================================

# Bu değerler artık ilgili sekmelerde kontrol edilecek
yil_baslangic = 2015
yil_bitis = 2035
secili_senaryolar = ["Referans Senaryo (BAU)", "Sıkı ETS"]
...
# =============================================================================
# TEMA TOGGLE (Sağ üst köşe - minimal)
# =============================================================================

col_spacer, col_tema = st.columns([6, 1])

with col_tema:
    # Tema Toggle
    tema_secim = st.toggle("🌙", value=(st.session_state.tema == 'dark'), key="tema_toggle", help="Karanlık/Aydınlık tema")
...
with tab3:
    ...
    with col_ayar:
        ...
        # Analiz Dönemi (Sidebar'dan taşındı)
...
with tab4:
    ...
    # =========================================================================
    # SİMÜLASYON PARAMETRELERİ PANELİ (Sidebar'dan taşındı)
    # =========================================================================
    
    with st.expander("⚙️ Simülasyon Parametreleri", expanded=False):
        # Senaryo Seçimi (Sidebar'dan taşındı)
        secili_senaryolar = st.multiselect(
            "📊 Karşılaştırılacak Senaryolar",
            senaryo_secenekleri,
            default=["Referans Senaryo (BAU)", "Sıkı ETS"],
            key="senaryo_secim_sim"
        )
        ...
        with col_param1:
            karbon_fiyati = st.number_input(
                ...
                value=25,
                step=5,
                help="ETS başlangıç karbon fiyatı"
            )
        ...
        with col_param2:
            cap_azalma = st.slider(
                ...
                value=2.1,
                step=0.1,
                help="Emisyon tavanının yıllık azalma oranı"
            )
        ...
        with col_param3:
            tesvik_miktari = st.number_input(
                ...
                value=50000,
                step=10000,
                help="MW başına teşvik miktarı"
            )
        
        # =====================================================================
        # GELİR GERİ DÖNÜŞÜM MEKANİZMASI (Revenue Recycling)
        # =====================================================================
        # Referans: Carbone & Rivers (2017). The impacts of unilateral climate
        # policy on competitiveness: evidence from computable general equilibrium
        # models. Review of Environmental Economics and Policy.
        # =====================================================================
        
        st.markdown("---")
        st.markdown("#### 💰 Gelir Geri Dönüşüm Mekanizması")
        st.caption("ETS'den elde edilen karbon gelirlerinin nasıl kullanılacağını seçin")
        
        gelir_donus_secenekleri = {
            "Hazine": "Gelirler hazineye aktarılır, genel bütçeye eklenir",
            "Yeşil Yatırım Fonu": "Gelirler yenilenebilir enerji ve temiz teknoloji yatırımlarına yönlendirilir",
            "Hanehalkı Transferi": "Gelirler düşük gelirli hanehalklarına doğrudan transfer edilir (karbon temettüsü)",
            "Firma Desteği": "Gelirler enerji-yoğun sektörlere geçiş döneminde destek olarak verilir"
        }
        
        gelir_donus_secimi = st.radio(
            "Gelir Geri Dönüşüm Senaryosu",
            list(gelir_donus_secenekleri.keys()),
            index=1,  # Varsayılan: Yeşil Yatırım Fonu
            help="Karbon gelirlerinin ekonomiye nasıl geri döneceğini belirler. "
                 "Bu seçim GDP etkisi, istihdam ve sosyal kabul açısından farklı sonuçlar üretir.",
            key="gelir_donus_radio"
        )
        
        # Seçim açıklaması
        st.info(f"📋 **{gelir_donus_secimi}:** {gelir_donus_secenekleri[gelir_donus_secimi]}")
        
        # Gelir dönüşüm oranı slider'ı
        col_gd1, col_gd2 = st.columns(2)
        
        with col_gd1:
            gelir_donus_orani = st.slider(
                "Geri Dönüşüm Oranı (%)",
                min_value=0,
                max_value=100,
                value=80,
                step=10,
                help="Karbon gelirlerinin yüzde kaçının geri dönüşüme tabi olacağı",
                key="gelir_donus_oran"
            )
        
        with col_gd2:
            # Tahmini gelir hesabı (basit)
            tahmini_gelir = (toplam_emisyon * karbon_fiyati * gelir_donus_orani / 100) / 1e6
            st.metric(
                "📊 Tahmini Yıllık Geri Dönüşüm",
                f"${tahmini_gelir:.1f} Milyar",
                help="Seçili parametre ve oranlarla tahmini geri dönüşüm miktarı"
            )
        ...
        with col_btn2:
            simule_et = st.button("🚀 Simülasyonu Çalıştır", use_container_width=True, type="primary")
            if simule_et:
                with st.spinner("🔄 Simülasyon çalışıyor... Bu işlem birkaç dakika sürebilir."):
                    try:
                        import subprocess
                        import sys
                        ...
                        result = subprocess.run(
                            [sys.executable, os.path.join(SCRIPT_DIR, "ajan_tabanli_simulasyon.py"),
                             "--mode", "all"],
                            capture_output=True,
                            text=True,
                            cwd=PROJECT_ROOT,
                        ...
                        if result.returncode == 0:
                            st.success("✅ Simülasyon başarıyla tamamlandı! Sonuçlar güncellendi.")
                            st.balloons()
                            
                            # Detaylı çıktıyı göster (opsiyonel)
                            with st.expander("📜 Simülasyon Çıktısı"):
                                st.code(result.stdout[-3000:] if len(result.stdout) > 3000 else result.stdout)
                            
                            # Sayfayı yenile
                            st.rerun()
                        else:
                            st.error("❌ Simülasyon başarısız oldu!")
                            with st.expander("🔍 Hata Detayları"):
                                st.code(result.stderr)
                    
                    except subprocess.TimeoutExpired:
                        st.error("⏱️ Simülasyon zaman aşımına uğradı (5 dakika). Terminal üzerinden çalıştırmayı deneyin.")
                    except Exception as e:
                        st.error(f"❌ Hata: {str(e)}")
                        st.info("💡 Alternatif: Terminal'de `python src/ajan_tabanli_simulasyon.py` komutunu çalıştırın.")
    ...
    if senaryo_sonuclari:
        ...
        st.code("python src/ajan_tabanli_simulasyon.py", language="bash")
...
with tab5:
    ...
    if df_il is not None and not df_il.empty:
        ...
        # --- SİMÜLASYON VERİSİ ENTEGRASYONU (v4.5) ---
        # Seçili senaryonun karbon fiyatını al
        mevcut_fiyat = karbon_fiyati  # Sidebar'dan gelen değer
        il_bazli_emisyon = {}  # Simülasyondan gelen il bazlı emisyonlar
        
        if senaryo_sonuclari:
            # En son senaryo sonucundan fiyat ve il emisyonlarını al
            for senaryo_adi in secili_senaryolar:
                if senaryo_adi in senaryo_sonuclari:
                    ...
                    # İl bazlı emisyon JSON varsa parse et
                    if 'Il_Emisyonlari_JSON' in secili_df.columns:
                        try:
                            import json
                            il_bazli_emisyon = json.loads(secili_df['Il_Emisyonlari_JSON'].iloc[-1])
                        except:
                            pass
                    break
        ...
        # Öncelik: Simülasyondan gelen il bazlı veri, yoksa sanayi payı ile hesapla
        if il_bazli_emisyon:
            df_harita['Simule_Emisyon'] = df_harita['Il_Adi'].map(
                lambda x: il_bazli_emisyon.get(x, df_harita.loc[df_harita['Il_Adi']==x, 'Sanayi_Payi'].values[0] * toplam_sim_emisyon if len(df_harita[df_harita['Il_Adi']==x]) > 0 else 0)
            )
        else:
        ...
        # --- ANA HARİTA: CHOROPLETİ İl Sınırları İçinde (v4.1) ---
        # GeoJSON dosyasını yükle
        geojson_path = os.path.join(PROJECT_ROOT, "data", "tr_illeri.geojson")
        ...
        if os.path.exists(geojson_path):
            ...
            # GeoJSON'daki il adlarını CSV ile eşleştir
            il_adi_map = {
                ...
                'İçel': 'Mersin', 'İstanbul': 'Istanbul', 'İzmir': 'Izmir',
                ...
                'Karaman': 'Karaman', 'Kırıkkale': 'Kirikkale', 'Batman': 'Batman',
                'Şırnak': 'Sirnak', 'Bartın': 'Bartin', 'Ardahan': 'Ardahan',
                'Iğdır': 'Igdir', 'Yalova': 'Yalova', 'Karabük': 'Karabuk',
                'Kilis': 'Kilis', 'Osmaniye': 'Osmaniye', 'Düzce': 'Duzce'
            }
            
            # GeoJSON feature'lara karbon maliyeti ekle
            for feature in turkey_geojson['features']:
                geojson_il = feature['properties']['name']
                csv_il = il_adi_map.get(geojson_il, geojson_il)
                
                # İl verisini bul
                il_row = df_harita[df_harita['Il_Adi'] == csv_il]
                if not il_row.empty:
                    feature['properties']['karbon_maliyeti'] = float(il_row['Karbon_Maliyeti_Milyon_USD'].values[0])
                    feature['properties']['emisyon'] = float(il_row['Simule_Emisyon'].values[0])
                    feature['properties']['risk_skoru'] = float(il_row['Risk_Skoru'].values[0])
                else:
                    feature['properties']['karbon_maliyeti'] = 0
                    feature['properties']['emisyon'] = 0
                    feature['properties']['risk_skoru'] = 0
            
            # Choropleth Harita oluştur
            fig_map = go.Figure(go.Choroplethmapbox(
                geojson=turkey_geojson,
                locations=[f['id'] for f in turkey_geojson['features']],
                z=[f['properties']['karbon_maliyeti'] for f in turkey_geojson['features']],
                colorscale='Reds',
                marker_opacity=0.7,
                marker_line_width=1,
                marker_line_color='white',
                colorbar=dict(
                    title=dict(text="Karbon Maliyeti<br>(Milyon $)", font=dict(color=tema['text_primary'])),
                    tickfont=dict(color=tema['text_primary']),
                    bgcolor='rgba(0,0,0,0)'
                ),
                hovertemplate=(
                    "<b>%{text}</b><br>" +
                    "Karbon Maliyeti: $%{z:.3f}M<br>" +
                    "<extra></extra>"
                ),
                text=[f['properties']['name'] for f in turkey_geojson['features']]
            ))
            
            fig_map.update_layout(
                mapbox_style="carto-positron",
                mapbox_zoom=5,
                mapbox_center={"lat": 39.0, "lon": 35.0},
                height=550,
                margin={"r": 0, "t": 40, "l": 0, "b": 0},
                title=dict(
                    text="🗺️ Türkiye İl Bazlı Karbon Maliyeti Haritası (81 İl)",
            ...
            # GeoJSON yoksa eski scatter haritayı kullan
            st.warning("⚠️ GeoJSON dosyası bulunamadı. Nokta haritası gösteriliyor.")
