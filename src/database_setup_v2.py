...
def veritabani_kurulumu():
    ...
    try:
        ...
        print("\n" + "-" * 40)
        print("TABLO 4: İl Dağılım Katsayıları (81 İl)")
        print("-" * 40)
        
        # =====================================================================
        # v2.1 GÜNCELLEME: 81 il verisi eklendi
        # Kaynak: TÜİK İl Bazında GSYH 2023, ADNKS 2023
        # =====================================================================
        
        # Önce 81 il dosyasını kontrol et, yoksa eski dosyayı kullan
        if os.path.exists("data/il_dagilim_katsayilari_81il.csv"):
            df_il = pd.read_csv("data/il_dagilim_katsayilari_81il.csv", comment='#')
            print(f"  ✅ 81 İl verisi yüklendi: {len(df_il)} il")
        else:
            df_il = pd.read_csv("data/il_dagilim_katsayilari.csv")
            print(f"  ⚠️ Eski veri kullanılıyor: {len(df_il)} bölge")
        
        df_il.to_sql("il_katsayilari", conn, if_exists="replace", index=False)
        
        # =====================================================================
        # 6.5. TESİS LİSTESİ (YENİ - v2.1)
        # =====================================================================
        # Kaynak: TEİAŞ 2024, EPDK 2024, Enerji Atlası
        # ETS kapsamına girecek büyük tesislerin listesi
        # =====================================================================
        
        print("\n" + "-" * 40)
        print("TABLO 5: Tesis Listesi (ETS Kapsamı)")
        print("-" * 40)
        
        if os.path.exists("data/tesisler.csv"):
            df_tesis = pd.read_csv("data/tesisler.csv", comment='#')
            df_tesis.to_sql("tesisler", conn, if_exists="replace", index=False)
            
            # Özet istatistikler
            toplam_kapasite = df_tesis['Kapasite_MW'].sum()
            toplam_emisyon = df_tesis['Yillik_Emisyon_tCO2'].sum() / 1e6  # Mt
            ets_tesis = df_tesis[df_tesis['ETS_Kapsami'] == 'Evet']
            
            print(f"  ✅ Tesis sayısı: {len(df_tesis)}")
            print(f"  ✅ Toplam kapasite: {toplam_kapasite:,.0f} MW")
            print(f"  ✅ Tahmini yıllık emisyon: {toplam_emisyon:.1f} Mt CO₂")
            print(f"  ✅ ETS kapsamındaki tesis: {len(ets_tesis)}")
        else:
            print("  ⚠️ tesisler.csv bulunamadı, tablo oluşturulmadı")
